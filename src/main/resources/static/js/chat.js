...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addAssistantMessage(content, citations) {
        ...
        if (content && content.length > 0) {
            ...
            let formattedContent = content[0].text;
            
            if (citations && citations.length > 0) {
                // Find all citation references in text using regex
                const matches = [];
                const pattern = /\[doc(\d+)\]/g;
                let match;
                
                // Create a Document Fragment to work with the text
                ...
                for (let i = matches.length - 1; i >= 0; i--) {
                    ...
                    if (citation) {
                        ...
                        // Create the HTML for the citation
                        const before = textContent.substring(0, m.position);
                        const after = textContent.substring(m.position + m.fullMatch.length);
                        const citationLink = `<a class="badge bg-primary rounded-pill" style="cursor: pointer;" data-message-id="${messageId}" data-index="${idx}">${idx}</a>`;
                        
                        // Replace text content
                        textContent = before + citationLink + after;
                    }
                }
    ...
    function sendMessage(text) {
        ...
        .then(data => {
            ...
            // Get citations directly from the response
            ...
            addAssistantMessage(formattedContent, citations);
